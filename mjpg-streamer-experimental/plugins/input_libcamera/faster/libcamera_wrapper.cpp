...
static void requestComplete(Request *request)
{
    ...
    if(jpeg_frame_count % 3 != 0)
    {
        request->reuse(
            Request::ReuseBuffers);
    ...
    for(auto &pair:request->buffers())
    {
        ...
        if(compress_yuv420_to_jpeg_fast(
              buffer,
              current_width,
              current_height,
              &jpeg,
...
// NV12 → JPEG用
u_buffer.resize(width / 2 * 8);
v_buffer.resize(width / 2 * 8);


// 初期JPEGサイズ
jpeg_work_buffer.resize(
    width * height * 3 / 2
);
